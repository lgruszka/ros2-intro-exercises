...
def plan_and_execute(robot, component, tries=3):
    ...
    for i in range(tries):
        ...
        if result:
            ...
            return True
        print(f'  plan nieudany (próba {i + 1}/{tries}) — ponawiam')
    return False
...
def goto_pose(robot, arm, x, y, z):
    ...
    goal.pose.orientation.x = 1.0   # chwytak w dół (180° wokół X)
...
def main():
    ...
    print('1/8 scena: stół + klocek')
    add_box(psm, 'stol', (0.8, 0.9, 0.05), (0.4, 0.0, -0.025))
    add_box(psm, 'klocek', (0.04, 0.04, 0.06), KLOCEK_XYZ)

    print('2/8 ready + otwarcie chwytaka')   # otwarte palce PRZED zjazdem — zamknięte
    assert goto_named(panda, arm, 'ready')   # to zwarta bryła, która koliduje z klockiem
    gripper(panda, hand, 'open')
    print('3/8 pre-grasp');        assert goto_pose(panda, arm, KLOCEK_XYZ[0], KLOCEK_XYZ[1], PREGRASP_Z)
    print('4/8 zjazd (otwarte palce obejmują klocek)')
    ...
    print('5/8 attach + chwyt');   set_attached(psm, 'klocek', attach=True)
    gripper(panda, hand, 'close')
    print('6/8 podnieś i przenieś')
    assert goto_pose(panda, arm, KLOCEK_XYZ[0], KLOCEK_XYZ[1], PREGRASP_Z)
    assert goto_pose(panda, arm, CEL_XYZ[0], CEL_XYZ[1], PREGRASP_Z)
    print('7/8 odłóż');            assert goto_pose(panda, arm, CEL_XYZ[0], CEL_XYZ[1], PLACE_Z)
    gripper(panda, hand, 'open')
    set_attached(psm, 'klocek', attach=False)
    print('8/8 powrót do ready');  goto_named(panda, arm, 'ready')
    print('✓ pick & place zakończony')
